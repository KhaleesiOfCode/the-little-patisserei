...
export default function MenuPage() {
  ...
  }, [activeCategory, search, foodType]);
      ...
      <section className="mx-auto max-w-7xl px-5 py-10">
        ...
        <div className="grid gap-6 lg:grid-cols-[260px_1fr]">
          <aside className="h-fit rounded-[2rem] bg-white p-4 shadow-sm ring-1 ring-[#F4CFC8] lg:sticky lg:top-24">
            <h2 className="mb-4 px-3 text-sm font-bold uppercase tracking-[0.2em] text-[#F08C9B]">
              Categories
            </h2>
            ...
            <div className="space-y-2">
              <button
                ...
                className={`flex w-full items-center justify-between rounded-2xl px-4 py-3 text-left text-sm font-semibold transition ${
                  activeCategory === "All"
                    ? "bg-[#F08C9B] text-white shadow-md"
                    : "bg-[#FFF8E4] text-[#3A2A2A] hover:bg-[#FADCD4]"
                }`}
              >
                <span>All</span>
                <span className="rounded-full bg-white px-2 py-0.5 text-xs text-[#F08C9B]">
                  {totalItems}
                </span>
              </button>
          ...
          <div>
            <div className="mb-6 rounded-[2rem] bg-white p-4 shadow-sm ring-1 ring-[#F4CFC8]">
              <div className="grid gap-4 md:grid-cols-[1fr_auto]">
                <input
                  ...
                  className="rounded-full border border-[#F4CFC8] bg-[#FFF8E4] px-5 py-3 text-[#3A2A2A] outline-none placeholder:text-[#7A6262] focus:border-[#F08C9B]"
                />
                    ...
                    <button
                      ...
                      className={`rounded-full px-4 py-2 text-sm font-bold transition ${
                        foodType === filter.value
                          ? "bg-[#F08C9B] text-white"
                          : "text-[#F08C9B] hover:bg-white"
                      }`}
                    >
                      {filter.label}
                    </button>
            ...
            <div className="mb-5 flex items-end justify-between">
              <div>
                <p className="text-sm font-bold uppercase tracking-[0.22em] text-[#F08C9B]">
